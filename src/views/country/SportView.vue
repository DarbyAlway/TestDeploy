...
<template>
  <!-- <p v-for="sport in country.sports" :key="sport.name">
        {{ sport.name }}: 
        Total Medals: {{ sport.total }},
        Bronze: {{ sport.bronze }},
        Silver: {{ sport.silver }},
        Gold: {{ sport.gold }}
      </p> -->
  ...
  <div class="overflow-x-auto px-4 sm:px-28 md:px-32 lg:px-80 xl:px-96">
    <table class="table">
      ...
      <tbody>
        <!-- row 1 -->
        <tr v-for="sport in country.sports" :key="sport.name">
          <td class="text-center">{{ sport.name }}</td>
          <td class="text-center">{{ sport.gold }}</td>
          <td class="text-center">{{ sport.silver }}</td>
          <td class="text-center">{{ sport.bronze }}</td>
          <td class="text-center">{{ sport.total }}</td>
        </tr>
        <!-- row 2 -->
      </tbody>
    </table>
